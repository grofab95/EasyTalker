import { deleteTokens, getAccessToken, getTokens, saveTokens } from './authUtils'
...
const refreshToken = async () => {
    ...
    const requestResult = await apiClient
        .post<ApiResponse<AuthenticationResult>>('authentication/refresh-token', {
...
const ensureNonExpiredTokens = async () => {
    ...
    try {
        const decodedAccessToken = decodeToken(getAccessToken()) 
        if (!decodedAccessToken) {
            setTimeout(ensureNonExpiredTokens, DEFAULT_TIMEOUT)
            return
        }
        
        const secondsToExpiration = (decodedAccessToken.exp * 1000 - Date.now()) / 1000
